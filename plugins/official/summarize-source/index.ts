...
export default async (request: Request) : Promise<Response> => {
  ...
  if (!source) {
    return new Response("Source field empty", { status: 400 });
  }

  const html = await fetchWebPage(source);
  const text = extractContent(html);
  if (text.length == 0) return new Response("Failed to extract text from source", { status: 400 })
  const systemPrompt = metadata || "Summarize the following text with the most unique and helpful points, into a bullet list of key points and takeaways";
